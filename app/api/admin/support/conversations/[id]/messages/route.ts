...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const limited = rateLimitRequest({ key: `admin-support-messages:${meta.ip}`, limit: 100, windowMs: 60_000, operation: "admin-support-messages" });
  ...
  try {
    ...
    const page = await listSupportMessages({ conversationId: params.data.id, cursor: request.nextUrl.searchParams.get("cursor"), limit: 40 });
    return apiSuccess(page);
  } catch (error) {
    logSafeError("admin.support.messages.get", error);
    return apiError("INTERNAL_ERROR", "Unable to load support messages right now.");
  }
}

export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  const meta = requestMeta(request);
  const limited = rateLimitRequest({ key: `admin-support-reply:${meta.ip}`, limit: 50, windowMs: 60_000, operation: "admin-support-reply" });
  if (limited) return limited;
  ...
  try {
    ...
    logSafeError("admin.support.messages.post", error);
    return apiError("INTERNAL_ERROR", "Unable to send support reply right now.");
  }
}

...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const removal = await removeBackgroundWithPhotoRoom({ buffer: source, filename: `wardrobe-${id}.webp`, mimeType: sourceContentType });
    if (!removal.ok) {
    ...
    const item = await WardrobeItem.findOneAndUpdate(
      ...
      { $set: {
        ...
        "images.front.backgroundRemovalVersion": photoRoomBackgroundRemovalVersion,
        "images.front.backgroundRemovalError": null,

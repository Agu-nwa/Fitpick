...
export function OutfitDetailClient({ id, mockOutfit }: { id: string; mockOutfit?: OutfitRecommendation }) {
  const session = useSession();
  const [outfit, setOutfit] = useState<OutfitRecommendation | null>(mockOutfit || null);
  ...
  useEffect(() => {
    if (session.status === "authenticated") void loadOutfit();
  }, [loadOutfit, session.status]);

  if (session.status === "loading" || status === "loading" || (session.status === "authenticated" && status === "idle" && !mockOutfit)) {
  ...
  if (session.status === "logged-out") {
    return (
      <>
        <OutfitAuthRequiredState />
        {mockOutfit ? <OutfitResult outfit={mockOutfit} /> : null}
      </>
    );
  }
  ...
  if (session.status === "backend-unavailable" || status === "unavailable") {
    return (
      <>
        <OutfitBackendUnavailableState onRetry={session.status === "backend-unavailable" ? session.refresh : loadOutfit} />
        {mockOutfit ? <OutfitResult outfit={mockOutfit} /> : null}
      </>
    );
  }

  if (status === "not-found" || !outfit) return <OutfitNotFoundState />;
  if (status === "error") return <OutfitApiErrorState onRetry={loadOutfit} />;

  return <OutfitResult outfit={outfit} canSwap={canSwap} onOutfitChange={setOutfit} />;
}

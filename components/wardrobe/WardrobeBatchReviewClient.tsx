...
export function WardrobeBatchReviewClient({ batchId }: { batchId: string }) {
  ...
  return <div className="mt-7 space-y-5">
    <Card className="border-warning/30 bg-warning/10"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold text-ink">❗ Review matters · {batch.completedCount || 0} of {batch.itemCount} saved</p><p className="mt-1 text-xs leading-5 text-muted">Confirm the detected details so Stylist can understand each piece and make accurate recommendations. Unconfirmed items are not used for outfits.</p></div><Badge tone={batch.status === "completed" ? "success" : "warning"}>{batch.status === "completed" ? "Complete" : "Review required"}</Badge></div></Card>

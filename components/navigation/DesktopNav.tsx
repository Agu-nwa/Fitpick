...
export function DesktopNav() {
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-72 flex-none flex-col border-r border-line bg-surface/70 px-6 py-7 backdrop-blur-xl lg:flex">
        ...
        {primaryNavItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {active ? <span className="ml-auto size-1.5 rounded-full bg-cocoa" /> : null}
            </Link>
          );
        })}
      </nav>

      <Link href="/outfit" className="group mt-auto overflow-hidden rounded-xl3 border border-olive/30 bg-ink p-5 text-canvas shadow-card transition hover:border-cocoa/50">

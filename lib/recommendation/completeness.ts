...
export function evaluateOutfitCompleteness(items: any[] = []): OutfitCompleteness {
  ...
  const hasTop = hasOnePiece || componentSet.has("top") || selected.some((item) => item.structureRole === "top" || item.category === "tops");
  const needsBottom = !hasOnePiece;
  const hasBottom = !needsBottom || componentSet.has("bottom") || selected.some((item) => item.structureRole === "bottom" || item.category === "bottoms");
  ...
  if (!footwearIncluded) completenessWarnings.push("No compatible shoes were selected for this look. Review your footwear details or add shoes to complete it.");

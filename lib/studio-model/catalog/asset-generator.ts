...
import { validateStudioModelAssetWithIdentity } from "./asset-validation";
...
import { loadStudioModelIdentityReference, resolveStudioModelIdentityReference } from "../identity-references";
...
export async function generateStudioModelAsset(configuration: unknown, version: string = STUDIO_MODEL_CATALOG_VERSION) {
  ...
  try {
    ...
    const identityReference = resolveStudioModelIdentityReference(appearance);
    const identityBody = identityReference && !useStub ? await loadStudioModelIdentityReference(identityReference) : undefined;
    const prompt = buildStudioModelPrompt(appearance, identityReference);
    const generated = useStub ? await deterministicStudioModelStub.generate(prompt) : null;
    const response = generated ? null : identityBody
      ? await openai.images.edit({ model, image: [await toFile(identityBody, `${identityReference?.id || "studio-model-identity"}.png`, { type: "image/png" })], prompt, size: "1024x1536", quality: "high", output_format: "png", input_fidelity: "high" })
      : await openai.images.generate({ model, prompt, size: "1024x1536", quality: "high", output_format: "png" });
    ...
    const validation = useStub ? { accepted:true,reviewRequired:false,qualityScore:1,checks:{stub:true},width:1024,height:1536,format:"png" } : await validateStudioModelAssetWithIdentity(body, appearance, identityBody);

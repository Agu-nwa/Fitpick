...
function avatarFingerprint(profile: any, options: AvatarPreviewOptions = {}) {
  return {
    ...
    studioModelConfiguration: profile.studioModelConfiguration || null,
    studioModelAppearanceKey: profile.studioModelAppearanceKey || null,
...
export function serializeAvatarPreview(preview: any) {
  ...
  return {
    ...
    requestedRoles: preview?.requestedRoles || [],
    providerSupportedRoles: preview?.providerSupportedRoles || [],
    partiallySupportedRoles: preview?.partiallySupportedRoles || [],
    unsupportedRoles: preview?.unsupportedRoles || [],
    ...
    validationStatus: preview?.validationStatus || "not_started",
    validationSafeReason: preview?.validationSafeReason || "",
    validationCheckedItemIds: (preview?.validationCheckedItemIds || []).map(String),
    validationMissingItemIds: (preview?.validationMissingItemIds || []).map(String),
    validationMismatchedItemIds: (preview?.validationMismatchedItemIds || []).map(String),
    validationAttempts: Number(preview?.validationAttempts || 0),
    validationLastAttemptAt: preview?.validationLastAttemptAt ? new Date(preview.validationLastAttemptAt).toISOString() : null,
